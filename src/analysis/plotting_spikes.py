# src/analysis/plotting_spikes.py
...
def _compute_grid_shape(n_units: int) -> Tuple[int, int]:
    """Finde (rows, cols) mit rows*cols >= n_units und möglichst quadratisch."""
    if n_units <= 0:
        return 0, 0
...
def _binned_spike_counts(
    ...
    """
    Spike-Counts pro Neuron und Zeit-Bin.

    times_ms : Spikezeiten (ms)
    idx      : 0-basierte Neuronenindizes (gleiche Länge wie times_ms)
    n_units  : Anzahl Neuronen
    ...
    np.add.at(counts, (idx, bin_idx), 1.0)

    return counts, t_centers


def plot_population_spikes_interactive(
    cfg: Dict[str, Any],
    data: Dict[str, Any],
    ...
    """
    Interaktive Visualisierung: Spike-Präsenz (0/1) pro Zeit-Bin.

    - Unteres Panel: E-Population als Heatmap in (rows_E x cols_E)-Layout.
    - Oberes Panel: inhibitorische Neuronen (IA_1, IH, IA_2) als gestapelte Blöcke.
    - Ein Slider + Play/Pause steuern den Zeit-Bin.

    Annahmen:
      - E-GIDs: 1 .. N_E
      - danach kommen IA_1, IH, IA_2.
    ...
    counts_E, t_bins = _binned_spike_counts(
        times_E,
        idx_E,
        n_units=N_E,
    ...
    n_bins = counts_E.shape[1]
    if n_bins == 0:
        print("No time bins (check t_start/t_stop/bin_size).")
        return

    # -----------------------
    # Inhibitorische Spikes (IA_1, IH, IA_2)
    # -----------------------
    ...
    if have_I:
        ...
        idx_I = senders_I[mask_I] - inh_first_gid  # 0..N_I_total-1

        counts_I_all, t_bins_I = _binned_spike_counts(
            times_I,
            idx_I,
            n_units=N_I_total,
            t_start_ms=t_start_ms,
        ...
        counts_IA1 = counts_I_all[0:N_IA_1, :]                           # (N_IA_1, n_bins)
        counts_IH  = counts_I_all[N_IA_1:N_IA_1 + N_IH, :]               # (N_IH, n_bins)
        counts_IA2 = counts_I_all[N_IA_1 + N_IH:N_I_total, :]            # (N_IA_2, n_bins)
    else:
        counts_IA1 = np.zeros((0, n_bins))
        counts_IH  = np.zeros((0, n_bins))
        counts_IA2 = np.zeros((0, n_bins))

    # -----------------------
    # Frames bauen (0/1: hat gespikt?)
    # -----------------------
    ...
    def _build_block_frames(counts_block: np.ndarray, cols: int) -> tuple[np.ndarray, int]:
        ...
        frames_flat[:, :N_block] = (counts_block.T > 0).astype(float)
        frames_block = frames_flat.reshape(n_bins, rows_block, cols)
        return frames_block, rows_block

    # Spaltenzahl für alle I-Blöcke gleich wählen (z.B. möglichst „quadratisch“ nach größtem Block)
    max_I_block = max(N_IA_1, N_IH, N_IA_2, 1)
    rows_Iref, cols_I = _compute_grid_shape(max_I_block)

    frames_IA1, rows_IA1 = _build_block_frames(counts_IA1, cols_I)
    frames_IH,  rows_IH  = _build_block_frames(counts_IH,  cols_I)
    frames_IA2, rows_IA2 = _build_block_frames(counts_IA2, cols_I)
    ...
    ax_E.set_title(f"E spikes, t ≈ {t_bins[0]:.1f} ms")
    ...
    if ax_I is not None and frames_I is not None:
        ...
        ax_I.set_title("I spikes (IA₁ / IH / IA₂)")
    ...
    def update(_):
        ...
        ax_E.set_title(f"E spikes, t ≈ {t_bins[idx]:.1f} ms")
    ...
    # Timer
    timer = fig.canvas.new_timer(interval=100)  # 100 ms pro Frame
